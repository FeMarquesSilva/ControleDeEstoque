...
# Função para buscar estoque resumido por usuário
def buscar_estoque_resumido(id_usuario):
    try:
        ...
        # Cria uma lista de dicionários a partir dos resultados
        estoque_list = [{
            "nome_produto": r.nome_produto,
            "categoria": r.categoria,
            "unidademedida": r.unidademedida,
            "data_validade": r.data_validade,
            "total_produto": r.total_produto
        } for r in resultados]
...
def realizar_entrada_estoque(id_usuario):
    ...
    if not data.get('produto_id') or not data.get('numero_lote') or not data.get('validade') or not data.get('quantidade'):
        return jsonify({'erro': 'Preencha todos os campo obrigatórios.'}), 400
    
    if data.get('quantidade') <= 0:
        return jsonify({'erro': 'Quantidade precisa ser positiva e maior que zero.'}), 400
